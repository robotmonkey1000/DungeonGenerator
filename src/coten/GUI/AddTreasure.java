...
public class AddTreasure extends Stage {
    ...
    /**
     * Main Constructor for adding a treasure.
     * @param parentStage The stage who called this gui (should be an edit panel)
     * @param c The controller so it can communicate.
     * @param ch If this is for chamber or not.
     */
    public AddTreasure(final Edit parentStage, final Controller c, boolean ch) {
        this.controller = c;
        ...
        initialize();
        this.setTitle("Add Treasure");
    }

    /**
     * Sets the roll for the treasure to be added.
     * @param index The specific roll.
     */
    ...
    private void initialize() {
        ...
        Text title = new Text("Pick your treasure.");
        bp.setTop(title);
        ...
        Button submit = new Button("Add Treasure");
    ...
    private void createChange(Change.SPACE space) {
        change = new Change(controller, Change.CHANGE_TYPE.ADD_TREASURE, space, controller.getTreasureRoll(selection.getSelectionModel().getSelectedItem()));
    ...
    private void successResponse(Text response) {
        ...
        response.setText("Treasure set to be added! (Remember to submit!)");
    }

    private void setUpMonsters() {
        for (String key: controller.getTreasures().keySet()) {
    ...
    private void failedResponse(Text response) {
        ...
        response.setText("Invalid Entry.");
    }
